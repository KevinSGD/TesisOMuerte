...
def build_model(cfg, data):
    ...
    if data.get("Gc_override") is not None:
        ...
        for c in cursos_activos:
            gnum = int(gcv[c])
            if gnum < 1:
                raise ValueError(f"Curso {c}: numero de grupos debe ser >= 1.")
            Gc[c] = gnum
            if gnum >= 3:
                cursos_con_3.add(c)
    ...
    pref = {(p, c): random.randint(0, 10) for c in cursos_activos for p in candidatos_por_curso[c]}
    ...
    for (c, gi) in G:
        ...
        if not permitidos:
            raise ValueError(
                f"Sin salon factible para {materias[c]}-G{gi+1} "
    ...
    model.Minimize(200 * (max_carga_p - min_carga_p) + 20 * sum(dev) - 1 * pref_expr)
